...
const ChatSidebar = ({
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className="fixed left-0 top-0 bottom-0 w-72 bg-card border-r border-border z-40 flex flex-col animate-sidebar-in shadow-2xl">
        {/* Header */}
        ...
        <div className="px-3 pt-3 pb-1">
          <button
            ...
            className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl border border-dashed border-border text-sm text-muted-foreground hover:text-foreground hover:bg-secondary hover:border-primary/30 transition-all duration-200"
          >
          ...
          {loading ? (
            <div className="space-y-2 px-1 pt-2">
              {[...Array(4)].map((_, i) => (
                <div
                  key={i}
                  className="h-10 rounded-lg bg-secondary animate-pulse-soft"
